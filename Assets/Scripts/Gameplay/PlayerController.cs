...
public class PlayerController : MonoBehaviour
{
    ...
    int UpDownAimingMultiplier = 1;
    [SerializeField] bool isUsingPunishment = true;
    ...
    void Update()
    {
        ...
        cameraRotation = new Vector3(cameraRotation.x + rotate.y * UpDownAimingMultiplier, cameraRotation.y + rotate.x, cameraRotation.z);
        ...
        if(isAttacking && animator.GetCurrentAnimatorStateInfo(1).normalizedTime >= animationFinishTime)
        {
            ...
            {
                sword.Checked();
                if(isUsingPunishment)
                {
                    if (sword.isInverting)
                    {
                        UpDownAimingMultiplier = -1;
                    }
                    else
                    {
                        UpDownAimingMultiplier = 1;
